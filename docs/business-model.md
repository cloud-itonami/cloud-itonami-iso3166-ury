# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Uruguay

## Classification

- Repository: `cloud-itonami-iso3166-ury`
- ISO 3166: `URY` (Uruguay)
...
- Social impact: [:uruguayan-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Uruguayan
  public contract
...
- registration walkthrough for Compras Estatales (comprasestatales.gub.uy),
  the state procurement portal operated by ARCE (Agencia Reguladora de
  Compras Estatales), and RUPE (Registro Único de Proveedores del
  Estado), the centralized supplier registry
- business/tax registration checklist: an active RUT (tax ID) and
  up-to-date standing with the DGI (Dirección General Impositiva) tax
  authority, then RUPE registration at proveedores.gub.uy (free, 5-year
  validity)
- local-content / preferential-procurement navigation: Uruguayan
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Uruguayan-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Uruguay).
