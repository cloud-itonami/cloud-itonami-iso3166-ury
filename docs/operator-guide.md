...
2. Register the client's intake: business type, target public function,
   prior filing history in Uruguay if any.
3. Run the advisor in read-only mode against Compras Estatales
   (comprasestatales.gub.uy).
4. Compare the checklist against the client's current documentation
   (RUT status with DGI, RUPE registration).
...
- named referral relationship with Uruguayan-licensed counsel or a
...
a working referral relationship with Uruguayan-licensed counsel or a
registered agent for whatever licensed representation the law of
Uruguay requires for actual public-procurement filings.
